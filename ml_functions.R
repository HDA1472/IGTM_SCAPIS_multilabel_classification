...
lasso_pca <- function(join_data,
  ...
  recipe <- recipe(formula, data = train_set) |> 
    ...
    step_zv(all_predictors(), -has_role("id")) |> 
    step_normalize(all_numeric(), -all_outcomes(), -has_role("id"), -all_of(binary_cols)) |> 
    step_pca(all_numeric(), num_comp = npcs, id = "my_pca")
  
  # Prepare the recipe (calculate PCA on training data)
  prepared_pca_recipe <- prep(recipe, training = train_set)
  
  pca_data <- juice(prepared_pca_recipe)
  pca_loadings <- tidy(prepared_pca_recipe, id = "my_pca")
  pca_variance <- prepared_pca_recipe |> 
    tidy(type = "variance", id = "my_pca") |> 
