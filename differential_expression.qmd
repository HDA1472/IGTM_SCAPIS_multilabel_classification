...
```{r}
volcanos_all <- lapply(disease_vec, function(disease) {
    de_res <- res[[disease]]$volcano_plot
  })

names(volcanos_all) <- disease_vec
```
...
top_proteins <- de_all |> 
  ...
  group_by(Disease) |>
  arrange(Disease, desc(abs(logFC))) |>
  slice_head(n = 1) |> 
  ungroup() |> 
...
plot_data <- data_igt |> 
  ...
  )) #|> mutate(Protein_disease = paste(Protein, Disease, sep = "_"))
