...
```{r}
features <- lasso_features_de |> 
  filter(Disease %in% c("MASLD", "MS", "Obesity")) |>
  group_by(Disease) |> 
  arrange(Disease, desc(Scaled_Importance)) |>
  slice_head(n = 5)
...
interest_groups <- c("MASLD", "MS_MASLD", "Obesity_MS", "None")
...
radar_dat <- radar_dat |> slice(match(c("1", "2", c("MASLD", "MS_MASLD", "Obesity_MS", "None")), rownames(radar_dat)))
color_order <- palette_groups[c("MASLD", "MS_MASLD", "Obesity_MS", "None")]
colors_alpha <- sapply(color_order, alpha, alpha = 0.5)

plot_filename <- paste0("results/plots/radar_plot_", "MASLD_others", ".png")
