...
res <- lapply(disease_vec, function(disease) {
  ...
  discovery_cohort <- data_igt |>
    select(-all_of(cols_to_exclude)) |> 
    select(-any_of(metadata_cols), -Sleep_apnea)

  model_res <- lasso_de(discovery_cohort, metadata, disease, cv_sets = 10, cor_threshold = 0.9, grid_size = 30, ntop=50)
  boxplot <- plot_protein_boxplot(data_igt, disease, model_res$features, points = FALSE)
  return(c(model_res, list("boxplot" = boxplot)))
})
...
```{r}
saveRDS(res, file="results/models/lasso_binary_onlydeproteins_50.RData")
```
...
saveRDS(features, file="results/models_results/lasso_onlydeproteins_features_50.RData")
saveRDS(auc, file="results/models_results/lasso_onlydeproteins_auc_50.RData")
```

### Validate on SCAPIS

```{r}
lasso_binary_onlydeproteins <- readRDS("results/models/lasso_binary_onlydeproteins_30.RData")
```
...
```{r}
auc_discovery <- readRDS("results/models_results/lasso_auc.RData")
auc_validation <- readRDS("results/models_results/lasso_validate_auc.RData")
auc_all <- bind_rows(auc_discovery |> mutate(Set = "Discovery"), 
                     auc_validation |> mutate(Set = "Validation"))

auc_all |>
  ggplot(aes(x = Disease, y = AUC, fill = Set)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(AUC, 2)), position = position_dodge(width = 1), vjust = -0.5) +
  scale_fill_manual(values = c("Discovery" = "purple1", "Validation" = "pink")) +
  labs(y = "AUC") +
  theme_hpa() +
