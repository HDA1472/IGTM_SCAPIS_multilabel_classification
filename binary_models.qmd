...
res <- lapply(disease_vec, function(disease) {
  ...
  validation_cohort <- data_scapis |>
    select(-all_of(cols_to_exclude)) |> 
    select(all_of(names(discovery_cohort)))
  
  model_res <- validate_model(lasso_binary[[disease]]$final_wf, discovery_cohort, validation_cohort, disease, disease_palette)
...
auc_all <- lapply(disease_vec, function(disease) {
    auc <- res[[disease]]$auc |> as_tibble() |> rename(AUC = value) |> mutate(Disease = disease)
  })

auc_all <- bind_rows(auc_all)
saveRDS(auc_all, file="results/models_results/lasso_validate_auc.RData")
```
...
```{r}
nfeatures_disease = c("Obesity" = 25, "IGT" = 25, "T2D" = 15, "MS" = 20, "MASLD" = 10, "Carotid_plaque" = 10, "CAC" = 15, "Sleep_apnea" = 20)
...
res <- lapply(disease_vec, function(disease) {
  ...
  model_res <- lasso_de(discovery_cohort, metadata, disease, cv_sets = 10, cor_threshold = 0.9, grid_size = 30, ntop=nfeatures_disease[[disease]])
  boxplot <- plot_protein_boxplot(data_igt, disease, model_res$features, points = FALSE)
  return(c(model_res, list("boxplot" = boxplot)))
})
...
```{r}
saveRDS(res, file="results/models/lasso_binary_onlydeproteins_depanel.RData")
```
...
saveRDS(features, file="results/models_results/lasso_onlydeproteins_features_depanel.RData")
saveRDS(auc, file="results/models_results/lasso_onlydeproteins_auc_depanel.RData")
```

### Validate on SCAPIS

```{r}
lasso_binary_onlydeproteins <- readRDS("results/models/lasso_binary_onlydeproteins_depanel.RData")
```
...
res <- lapply(disease_vec, function(disease) {
  ...
  model_res <- validate_model(lasso_binary_onlydeproteins[[disease]]$final_wf, discovery_cohort, validation_cohort, disease, disease_palette)
...
```{r}
# heatmap_disease_palette <- list(Obesity = c("0" = "white", "1" = "#A6CEE3"), 
...
#   pivot_longer(cols = -DAid, names_to = "PCs", values_to = "Value") |> 
#   left_join(data_igt |> select(all_of(c("DAid", disease_vec)))) |> 
#   tidyheatmap(
#     rows = PCs,
#     columns = DAid,
#     values = Value,
...
#     annotation_colors = heatmap_disease_palette,
...
```{r}
npcs <- c(100, 120, 140, 160, 180, 200)
res_pca <- lapply(npcs, function(npc) {
  print(paste("PCs:", npc))
  res <- lapply(disease_vec, function(disease) {
    print(paste(disease, "model:"))
    cols_to_exclude <- disease_vec |> setdiff(disease)
    ...
    model_res <- lasso_pca(discovery_cohort, disease, cv_sets = 10, grid_size = 30, npcs = npc)
...
extract_model_pca_res <- function(model, npcs) {
  auc_all <- lapply(npcs, function(npc) {
    auc <- model[[as.character(npc)]] |> mutate(nPC = npc)
  })
  return(bind_rows(auc_all))
}

res_pca_all <- extract_model_pca_res(res_pca, npcs)

saveRDS(res_pca_all, file="results/models_results/lasso_pcs_100_200_auc.RData")
```
...
plot_data <- data_igt |> 
  ...
  mutate(Disease_Value = case_when(
    ...
    Disease_Value == 0 ~ "Control",
    TRUE ~ "Control"  # Default case for NA values
  ))

plot_data |> 
  ggplot(aes(x = factor(Disease_Value, 
                        levels = c("Control", unique(Disease[!is.na(Disease_Value)]))), 
             y = Value, 
             fill = Disease_Value)) +
...
## Proteomics + clinical - discovery vs validation

```{r}
auc_discovery <- readRDS("results/models_results/lasso_onlydeproteins_auc_depanel.RData")
auc_validation <- readRDS("results/models_results/lasso_validate_auc.RData")
auc_all <- bind_rows(auc_discovery |> mutate(Set = "Discovery - IGTM"), 
                     auc_validation |> mutate(Set = "Validation - SCAPIS"))
...
features_per_model <- lasso_diseases_features |> 
...
node_sizes <- lasso_diseases_features |> 
  filter(Scaled_Importance >= importance_score) |>
  group_by(Disease) |> 
...
```{r}
lasso_pca_auc <- readRDS("results/models_results/lasso_pcs_20_200_auc.RData")
lasso_auc <- readRDS("results/models_results/lasso_auc.RData")
```

```{r}
lasso_pca_auc |> 
  ggplot(aes(x = nPC, y = AUC, color = Disease)) +
  geom_line() +
  geom_point() +
  geom_hline(data = lasso_auc, aes(yintercept = AUC), linetype = "dashed") +
  scale_color_manual(values = disease_palette) +
  labs(x = "Number of PCs", y = "AUC") +
  facet_wrap(~Disease) +
  theme_hpa()
```
...
lasso_de_all |> 
  ...
  geom_line() +
  geom_point() +
  geom_hline(data = lasso_onlyproteins_auc, aes(yintercept = AUC), linetype = "dashed") +
  scale_color_manual(values = disease_palette) +
