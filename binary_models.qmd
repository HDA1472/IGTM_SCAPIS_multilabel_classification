...
res <- lapply(disease_vec, function(disease) {
  ...
  model_res <- lasso(discovery_cohort, disease, cv_sets = 2, cor_threshold = 0.9, grid_size = 10)
...
res <- lapply(disease_vec, function(disease) {
  ...
  model_res <- elasticnet(discovery_cohort, disease, cv_sets = 2, cor_threshold = 0.9, grid_size = 10)
  boxplot <- plot_protein_boxplot(data_igt, disease, model_res$features, points = FALSE)
  return(c(model_res))
})
...
res <- lapply(disease_vec, function(disease) {
  ...
  model_res <- rf(discovery_cohort, disease, cv_sets = 2, cor_threshold = 0.9, grid_size = 5)
  boxplot <- plot_protein_boxplot(data_igt, disease, model_res$features, points = FALSE)
  return(c(model_res))
})
...
```{r}
saveRDS(res, file="results/models/rf_binary.RData")
```
...
```{r}
binary_lasso <- readRDS("results/models/lasso_binary.RData")
binary_elasticnet <- readRDS("results/models/elasticnet_binary.RData")
binary_rf <- readRDS("results/models/rf_binary.RData")
```