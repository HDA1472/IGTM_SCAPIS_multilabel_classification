...
res <- lapply(disease_vec, function(disease) {
  ...
  model_res <- lasso_pca(discovery_cohort, disease, cv_sets = 10, grid_size = 30, npcs = 200)
...
```{r}
saveRDS(res, file="results/models/lasso_binary_pcs_80.RData")
```
...
saveRDS(features, file="results/models_results/lasso_pcs_80_features.RData")
saveRDS(auc, file="results/models_results/lasso_pcs_80_auc.RData")
```
