...
```{r}
age_features <- ndiseasemodel$features |> filter(Scaled_Importance > 50) |> pull(Variable)
```

```{r}
umap_res_features <- do_umap(data_igt |> select(DAid, any_of(age_features), -Sex, -Smoke_status),
        metadata |> 
          filter(DAid %in% data_igt$DAid) |> 
          left_join(disease_count) |> 
          mutate(n = factor(n),
...
```{r}
pca_res_features <- do_pca(data_igt |> select(DAid, any_of(age_features), -Sex, -Smoke_status),
        metadata |> 
          filter(DAid %in% data_igt$DAid) |> 
          left_join(disease_count) |> 
          mutate(n = factor(n),
...
```{r}
protein_sets <- list(Pandisease_panel = proteomics_features_list, Age_panel = age_features)

# Create a Venn diagram with counts and set names inside the circles
venn.plot <- venn.diagram(
  x = protein_sets,
  category.names = c("Pan-disease Panel", "Age Panel"),
  filename = NULL,
  output = TRUE,
...
```{r}
common_proteins <- intersect(proteomics_features_list, age_features)
