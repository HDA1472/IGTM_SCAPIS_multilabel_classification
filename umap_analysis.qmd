...
cac_features_50 <- lasso_features |> 
  filter(Disease == "CAC") |> 
  filter(Scaled_Importance > 50) |> 
  pull(Variable)

do_umap(data_igt |> select(DAid, any_of(cac_features_50), -Sex, -Smoke_status),
        metadata |> mutate(CAC = factor(CAC)),
        color = "CAC",
        palette = c("1" = "#9E0142", "0" = "lightgrey"))
```
...
```{r}
do_umap(data_igt |> select(-Sex, -Smoke_status, -all_of(disease_vec)),
        metadata |> mutate(Sleep_apnea = factor(Sleep_apnea)),
        color = "Sleep_apnea",
        palette = c("1" = "#A6CEE3", "0" = "lightgrey"))
